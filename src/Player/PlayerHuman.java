...
public class PlayerHuman {
	 ...
	 */
	public void cardAction(Logic logic, SpecialFunction specialFunction, Controller controller, Deck deck, DisplayCards display) {
		boolean valid;
		int cardIndex = display.displayDeck(controller.getTopCard(), controller.getPlayer1().getHand());
		if (cardIndex > 0 ) {
			try {
			...
			if (valid) {							
				...
				System.out.println("Please enter a valid selection (Number between 0 and " + hand.size());
			}	
		}else if (cardIndex == 0) {
			deck.draw(1, hand);
		}
